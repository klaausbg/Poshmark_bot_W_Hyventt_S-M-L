...
const POSHMARK_URL =
  "https://poshmark.com/search?query=the%20north%20face%20hyvent&sort_by=added_desc&brand%5B%5D=The%20North%20Face&department=Women&category=Jackets_%26_Coats&size%5B%5D=S&size%5B%5D=M&size%5B%5D=L&price%5B%5D=-35&color%5B%5D=Black";
      ...
      if (item.title && item.price && item.size) {
        ...
        if (!hasFlaw) {
          if (firstMatch) {
            ...
            await sendTelegramMessage(
              "🔔 *You got new deals!*\n\nHere are the latest Women Hyvent Jackets:"
            );
            firstMatch = false;
          }
